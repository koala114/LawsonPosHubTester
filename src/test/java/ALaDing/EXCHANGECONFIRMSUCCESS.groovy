...
class EXCHANGECONFIRMSUCCESS extends Helper {
    ...
    def "call exchangeconfirm"() {
        // 兑换业务传券号；撤销业务不传券号
        given:
        // 券号结尾是0009或者券金额不是10元会返回 "responseCode":"0009","responseMessage":"券号结尾是0009或者券金额不正确"
        ExchangeConfirmRequest exchangeConfirm = createExchangeConfirmRequest(outTradeNo, coupons, barcodeResponse.total_fee, totalFee)
        when:
